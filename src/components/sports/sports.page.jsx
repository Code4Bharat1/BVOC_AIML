...
// ── Data ──────────────────────────────────────────────────────
const sports = [
  { id: "cricket",   name: "Cricket",    icon: MdSportsCricket,      color: "#007BFF", players: 11, status: "Active Season" },
  { id: "football",  name: "Football",   icon: MdSportsFootball,     color: "#FF7F00", players: 11, status: "Active Season" },
  { id: "chess",     name: "Chess",      icon: GiChessKnight,         color: "#9C27B0", players: 2,  status: "Tournament" },
  { id: "badminton", name: "Badminton",  icon: GiShuttlecock,color: "#00C853", players: 4,  status: "Active Season" },
  { id: "kabaddi",   name: "Kabaddi",    icon: MdSportsHandball,      color: "#FF2D55", players: 7,  status: "Off Season" },
];

const achievements = [
  { title: "Inter-College Cricket Champions", year: "2024", icon: FaTrophy, color: "#FF7F00" },
  { title: "District Chess Runner-Up", year: "2024", icon: GiChessKnight, color: "#9C27B0" },
  { title: "Best Sports College Award", year: "2023", icon: FaMedal, color: "#007BFF" },
  { title: "Football League Winners", year: "2023", icon: MdSportsFootball, color: "#00C853" },
];

const events = [
  { title: "Inter-Batch Cricket Tournament", date: "Jan 20", participants: 22, status: "completed", sport: "Cricket", icon: MdSportsCricket, color: "#007BFF" },
  { title: "Chess Championship Round 1", date: "Feb 14", participants: 10, status: "completed", sport: "Chess", icon: GiChessKnight, color: "#9C27B0" },
  { title: "Badminton Open Doubles", date: "Feb 28", participants: 16, status: "completed", sport: "Badminton", icon: GiShuttlecock, color: "#00C853" },
  { title: "Football League Season 2", date: "Mar 22", participants: 22, status: "upcoming", sport: "Football", icon: MdSportsFootball, color: "#FF7F00" },
  { title: "Kabaddi Friendly Match", date: "Apr 5", participants: 14, status: "upcoming", sport: "Kabaddi", icon: MdSportsHandball, color: "#FF2D55" },
];

const members = [
  { name: "Taha",    role: "Cricket Captain", initials: "TA", sport: MdSportsCricket,       c1: "#007BFF", c2: "#00C6FF" },
  { name: "Yaseen",  role: "Football Lead",   initials: "YA", sport: MdSportsFootball,      c1: "#FF7F00", c2: "#FFD200" },
  { name: "Zaid",    role: "Chess Champion",  initials: "ZA", sport: GiChessKnight,          c1: "#9C27B0", c2: "#FF4081" },
  { name: "Asif",    role: "Badminton",       initials: "AS", sport: GiShuttlecock, c1: "#00C853", c2: "#007BFF" },
  { name: "Faiz",    role: "Athletics",       initials: "FA", sport: FaFire,                 c1: "#FF2D55", c2: "#FF7F00" },
  { name: "Sudeep",  role: "Kabaddi",         initials: "SU", sport: MdSportsHandball,       c1: "#FF7F00", c2: "#FFD200" },
  { name: "Ahmed",   role: "Coach Assist",    initials: "AH", sport: GiWhistle,              c1: "#007BFF", c2: "#7B2FFF" },
  { name: "Vishesh", role: "Goalkeeper",      initials: "VI", sport: BsShieldFill,           c1: "#00C853", c2: "#007BFF" },
];

const stats = [
  { label: "Athletes",      value: 18, icon: FaUsers,            color: "#007BFF" },
  { label: "Sports",        value: 5,  icon: FaTrophy,           color: "#FF7F00" },
  { label: "Events Held",   value: 6,  icon: MdOutlineEmojiEvents,color: "#9C27B0" },
    ...
    <>
      ...
      <main className="min-h-screen bg-[#F4F7FC] relative overflow-hidden">
        ...
        <div
          ...
          style={{ backgroundImage: "radial-gradient(circle, #FF7F00 1px, transparent 1px)", backgroundSize: "28px 28px" }}
        />
        {/* Corner glows */}
        <div className="fixed -top-32 -left-32 w-96 h-96 rounded-full pointer-events-none z-0"
          style={{ background: "radial-gradient(circle, rgba(255,127,0,0.09), transparent 70%)" }} />
        <div className="fixed -bottom-24 -right-24 w-80 h-80 rounded-full pointer-events-none z-0"
          style={{ background: "radial-gradient(circle, rgba(0,123,255,0.07), transparent 70%)" }} />
        ...
        >
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
            ...
            <div className="mb-14">
              ...
              <p className="text-gray-400 text-sm mb-6">Click a sport to filter events</p>
                ...
                {sports.map((sp) => {
                  ...
                  return (
                    <button
                      ...
                      style={isActive
                        ? { background: `linear-gradient(135deg, ${sp.color}, ${sp.color}cc)`, borderColor: sp.color, boxShadow: `0 8px 24px ${sp.color}33` }
                        : {}}
                    >
                      <Icon className="text-2xl" style={{ color: isActive ? "#fff" : sp.color }} />
                      {sp.name}
                      <span
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-gray-700 text-sm font-semibold leading-tight">{ev.title}</p>
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          style={{ background: `linear-gradient(135deg, ${ach.color}, ${ach.color}bb)` }}
                        >
                          <AchIcon className="text-white text-xl" />
                        </div>
                        <div>
                          <p className="text-gray-700 text-sm font-bold leading-tight">{ach.title}</p>
                          <p className="text-gray-400 text-xs mt-0.5 flex items-center gap-1">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                {sports.map((sp, i) => {
                  ...
                  const maxWins = 8;
                  return (
                    <div key={sp.id} className="flex flex-col gap-2 p-4 rounded-xl border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-sm transition-all">
                      <div className="flex items-center justify-between">
                        <Icon className="text-xl" style={{ color: sp.color }} />
                        <span className="text-xs font-black" style={{ color: sp.color }}>{wins}W</span>
                      </div>
                      <p className="text-gray-700 text-xs font-bold">{sp.name}</p>
                      {/* Progress bar */}
                      <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full transition-all duration-700"
                          style={{ width: `${(wins / maxWins) * 100}%`, background: `linear-gradient(90deg, ${sp.color}, ${sp.color}99)` }}
                        />
                      </div>
                      <p className="text-gray-400 text-[10px]">{wins}/{maxWins} matches</p>
                    </div>
                  );
                })}
              </div>
                ...
                {members.map((m, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-gray-700 text-[11px] font-bold">{m.name}</p>
                        <p className="text-gray-400 text-[9px] mt-0.5 flex items-center justify-center gap-0.5">
                          <BsPersonFill className="text-[8px]" style={{ color: m.c1 }} />
                          {m.role}
                        </p>
                      </div>
                    </div>
            ...
            >
              <div className="absolute inset-0 opacity-[0.05]"
                style={{ backgroundImage: "repeating-linear-gradient(45deg, #FF7F00 0, #FF7F00 1px, transparent 0, transparent 50%)", backgroundSize: "16px 16px" }} />
              <FaTrophy className="text-6xl mx-auto mb-4 relative z-10" style={{ color: "#FF7F00" }} />
              ...
              <a
                ...
                style={{ background: "linear-gradient(135deg, #FF7F00, #FFD200)", boxShadow: "0 8px 24px rgba(255,127,0,0.35)" }}
              >
