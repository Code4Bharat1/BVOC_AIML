...
const DegreeComparisonTable = () => {
  const features = [
    {
      ...
      desc: "B.Voc focuses on practical, job-ready skills from day one so students graduate with real-world experience.",
      color: "blue"
    },
    {
      icon: <Zap className="w-10 h-10" />,
      title: "Faster Entry to Workforce",
      desc: "Students graduate and begin earning sooner compared to traditional degree programs.",
      color: "orange"
    },
    {
      icon: <Rocket className="w-10 h-10" />,
      title: "Modern Curriculum",
      desc: "Courses are designed with industry input to keep students aligned with modern technology and job market trends.",
      color: "blue"
    }
  ];
  ...
  return (
    <section
      aria-labelledby="degree-comparison-heading"
        ...
        <header className="text-center mb-16">
          ...
          >
            ...
            <span className="text-slate-900">
              The Clear Choice for Your Future
            </span>
          </h1>
          ...
          <div className="max-w-3xl mx-auto space-y-3">
            <p className="text-lg text-slate-700">
              <span className="font-semibold text-slate-900">
                Choosing the right course is one of the most important decisions of your career.
              </span>{" "}
            ...
            <p className="text-lg text-slate-600">
              ...
              <span className="font-semibold text-orange-500">B.Sc</span>{" "}
              provide theoretical foundations,{" "}
              <span className="font-semibold text-blue-600">B.Voc</span>{" "}
              focuses on{" "}
...
const FeatureCard = ({ icon, title, desc, color, index }) => {
  ...
  return (
    ...
    >
      {/* Glow */}
      <div
        className={`absolute -inset-0.5 bg-gradient-to-br ${styles.gradient} rounded-2xl opacity-0 group-hover:opacity-100 blur transition duration-300`}
      />

      {/* Card */}
      <div
        className={`relative bg-white border-2 ${styles.border} group-hover:${styles.hoverBorder} rounded-2xl p-8 shadow-sm group-hover:shadow-lg transition-all duration-300 group-hover:-translate-y-1`}
      >
        {/* Icon */}
        <div
          className={`${styles.bg} w-16 h-16 rounded-xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300`}
        >
        ...
        {/* Text */}
        <h2 className="font-bold text-xl mb-2 text-slate-900">
          {title}
        </h2>
        ...
        {/* Accent */}
        <div
          className={`mt-5 h-1 w-12 rounded-full bg-gradient-to-r ${
            color === "blue"
